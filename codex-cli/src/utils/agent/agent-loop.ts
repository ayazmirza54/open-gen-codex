...
import { isGeminiModel } from "../model-utils.js";
import { streamModelCompletion } from "../../components/llms/llm-orchestrator";
...
export class AgentLoop {
  ...
  public async run(
    ...
    if (isLoggingEnabled()) {
      log(`AgentLoop.run() invoked (generation=${this.generation})`);
    }
    ...
    // Increment the generation to handle stray events in case of
    // cancel/restart sequences.
    const ourGeneration = ++this.generation;

    // Abort all pending/stale callback IDs from previous runs.
    for (const id of this.pendingAborts) {
      input.push({
        type: "function_call_output",
        id,
        content: null, // This satisfies OpenAI but will not become real output
      });
    }

    this.pendingAborts.clear();

    const output: Array<ResponseInputItem> = [];

    // Add Gemini support - use the adapter if using a Gemini model
    if (isGeminiModel(this.model)) {
      try {
        // Convert history to a format the Gemini adapter can use
        ...
        for (const item of input) {
          if (item.type === "message") {
            ...
            // Process function outputs for Gemini
            if (item.output) {
              history.push({
                role: 'user',
                content: `Function result: ${item.output}`
              });
            }
          }
        }
        ...
        let lastMessage = "";
        for await (const message of streamModelCompletion(
          "", // Empty prompt since we've included everything in history
          this.model,
          history
        )) {
          lastMessage += message;
          ...
          // Look for function calls in text
          if (message.includes("[Function Call:")) {
            // Extract function name and arguments
            const matches = /\[Function Call: ([^\]]+)\]\n(\{.*\})/s.exec(lastMessage);
            if (matches && matches.length >= 3) {
              const functionName = matches[1];
              const functionArgs = matches[2];
              
              // Create a function call response item
              const functionCallId = `call_${randomUUID().replaceAll("-", "")}`;
              const functionCallItem = {
                id: functionCallId,
                type: "function_tool_calls",
                role: "assistant",
                function_tool_calls: [{
                  id: functionCallId,
                  type: "function",
              ...
              await this.handleFunctionCall(functionCallItem.function_tool_calls[0]);
            }
          }
        }
        ...
        console.error("Error with Gemini model processing:", error);
        this.onItem({
          id: randomUUID(),
          type: "message",
          role: "assistant",
          content: [{ 
            type: "text", 
            text: `Error with Gemini model: ${error instanceof Error ? error.message : 'Unknown error'}`
          }]
        });
        this.onLoading(false);
        return;
      ...
      try {
        ...
        for await (const event of stream) {
          ...
          if (event.type === "response.completed") {
            if (ourGeneration === this.generation && !this.canceled) {
